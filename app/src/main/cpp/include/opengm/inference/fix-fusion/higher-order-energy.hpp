...
private:
    ...
    size_t NumTerms() const {
        size_t numTerms = 0;
        for(int idxVr;idxVr<_varRecords.size();idxVr++){
            numTerms += _varRecords[idxVr]._terms.size();
        }
...
void HigherOrderEnergy<R, D>::_ReduceNegativeTerms(QR& qr) {
    ...
    int expectedEdges = 0;
    for(int idxVr=0;idxVr<_varRecords.size();idxVr++){
        expectedVars += _varRecords[idxVr]._higherOrderTerms;
        expectedEdges += _varRecords[idxVr]._quadraticTerms;
        expectedEdges +=  _varRecords[idxVr]._sumDegrees;
    }
    ...
    for(int idxVr=0;idxVr<_varRecords.size();idxVr++){
        VarRecord& vr=_varRecords[idxVr];
        typename std::list<Term>::iterator itTerm=vr._terms.begin();
        while(itTerm!=vr._terms.end()){
            Term& t=*itTerm;
            if (t.degree == 2) {
    ...
    for(int vrIdx=0;vrIdx<_varRecords.size();vrIdx++){
        qr.AddUnaryTerm(_varRecords[vrIdx]._id, 0, _varRecords[vrIdx]._coeff);
    }
}
